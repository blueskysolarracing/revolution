...
############# CONSTANTS FOR BMS #############
BATTERY_NUM_CELLS = 5
BATTERY_NUM_VOLTAGES = BATTERY_NUM_CELLS
BATTERY_NUM_STATE_OF_CHARGES = BATTERY_NUM_CELLS

BATTERY_NUM_TEMPERATURE_SENSOR = 3
BATTERY_NUM_TEMPERATURES = BATTERY_NUM_TEMPERATURE_SENSOR
...
class Battery:
    ...
    def __init__(self, battery_id: int, spi, gpio_b) -> None:
        self.ltc6810 = LTC6810(spi=spi, gpio_b=gpio_b)

        self.battery_id = battery_id
        self._current = 0.0
        self._state_of_charge_arr = [0.0] * BATTERY_NUM_STATE_OF_CHARGES
    ...
    def get_temperature(self, temperatures: list[float], get_mode: GetMode) -> None:
        ...
        for i in range(BATTERY_NUM_TEMPERATURES):
            if get_mode == Battery.GetMode.GET_MOST_RECENT:
                temperatures[i] = self._temperature_arr[i]
            elif get_mode == Battery.GetMode.GET_PAST_AVERAGE:
                temperatures[i] = get_queue_average(self._past_temp_queue)
            elif get_mode == Battery.GetMode.GET_FILTERED_RESULT:
                pass

    def get_voltage(self, voltages: list[float], get_mode: GetMode) -> None:

        for i in range(BATTERY_NUM_VOLTAGES):
            if get_mode == Battery.GetMode.GET_MOST_RECENT:
    ...
    def get_state_of_charge(self, state_of_charges: list[float]) -> None:
        for i in range(BATTERY_NUM_STATE_OF_CHARGES):
    ...
    def compute_soc(self) -> None:
        local_soc_arr = [0.0] * BATTERY_NUM_STATE_OF_CHARGES
